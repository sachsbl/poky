...
from poky_helpers import verify_s3_object, verify_s3_bucket
...
def test_verify_s3_object_exists_bad_key_returns_false():
    result = verify_s3_object(bucket=VALID_SOURCE_BUCKET, key='bad')

    assert result is False


def test_verify_s3_object_exists_bad_bucket_returns_false():
    result = verify_s3_object(bucket='bad', key=VALID_SOURCE_KEY)
